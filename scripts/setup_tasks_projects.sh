...
if (( $(oc get dc -n tasks-dev|wc -l) == 0 )); then
	oc new-app tasks-build/tasks:0.0-0 --name=tasks-dev --allow-missing-imagestream-tags=true -n tasks-dev
...
if (( $(oc get dc -n tasks-test|wc -l) == 0 )); then
	oc new-app tasks-dev/tasks:0.0-0 --name=tasks-test --allow-missing-imagestream-tags=true -n tasks-test
...
if (( $(oc get dc -n tasks-prod|wc -l) == 0 )); then
	oc new-app tasks-test/tasks:0.0-0 --name=tasks-prod --allow-missing-imagestream-tags=true -n tasks-prod
